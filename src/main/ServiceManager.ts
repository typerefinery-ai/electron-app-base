...
import { getPortFree, getProcessPathForPID, getProcessPidForPort, stopProcess } from './Utils'
...
class ServiceManager {
  ...
  async reload(restart = false) {
    ...
    this.#loadServices()
    this.#log('service manager update global env', this.elapsedTime(startReload))
    ...
    if (this.#serviceManagerEvents.sendServiceList) {
      this.#log('service manager send list of services to app', this.elapsedTime(startReload))
      this.#serviceManagerEvents.sendServiceList(this.#services)
    }

    // send globalenv to app
    if (this.#serviceManagerEvents.sendGlobalEnv) {
      this.#log('service manager send globalenv to app', this.elapsedTime(startReload))
      this.#serviceManagerEvents.sendGlobalEnv(this.#globalenv)
    }

    if (restart) {
      this.#log('service manager send startAll start', this.elapsedTime(startReload))
      await this.startAll()
      this.#log('service manager send startAll end', this.elapsedTime(startReload))
    }
  }
    ...
    glob //this loads all service.json files from services folder and its direct subfolders.
      ...
      .forEach((file: string) => {
        ...
        if (stat.isFile()) {
          // read service.json
          try {
            const serviceFileConfig: SericeConfigFile = JSON.parse(fs.readFileSync(file, 'utf8'))
  ...
  #addServicePort(port: number, serviceId = '', type = '', status = '', requested: number) {
  ...
  #updateServicePort(port: number, serviceId = '', type = '', status = '', newPort = -1) {
    //remove old port
    let oldSatus = ''
    if (this.#servicePorts[port + '']) {
  ...
  async reserveServicePort(port = 0, host = 'localhost', serviceId = '', type = '') {
  ...
  async startAll(forceInstall = false) {
    ...
    for (const service of this.#services) {
      ...
      this.#log(`service started ${service.id} : ${service.status} : ${service.isSetup}`)
    }
